...
namespace Tests.Legacy.Deserializer.Json
{
    ...
    public class ListTests
    {
        ...
        [Test]
        public void should_deserialize_simple_type_generic_enumerable()
        {
            const string json = @"[ 1, 2, 3 ]";
        ...
        [Test]
        public void should_fail_to_deserialize_simple_type_enumerable()
        {
            const string json = @"[ 1 ]";
            Assert.Throws<MappingException>(() => 
                Bender.Deserializer.Create().DeserializeJson<IEnumerable>(json))
                .InnerException.ShouldBeType<TypeNotSupportedException>();
        }

        [Test]
        public void should_fail_to_deserialize_simple_type_array_list()
        {
            const string json = @"[ 1 ]";
            Assert.Throws<MappingException>(() => 
                Bender.Deserializer.Create().DeserializeJson<ArrayList>(json))
                .InnerException.ShouldBeType<TypeNotSupportedException>();
        }

        [Test]
        public void should_deserialize_simple_type_array()
        {
            const string json = @"[ 1, 2, 3 ]";
        ...
        public void should_not_deserialize_simple_type_enumerable_property()
        {
            ...
            Assert.Throws<MappingException>(() => 
                Bender.Deserializer.Create().DeserializeJson<ListProperty>(json))
                .InnerException.ShouldBeType<TypeNotSupportedException>();
        }
