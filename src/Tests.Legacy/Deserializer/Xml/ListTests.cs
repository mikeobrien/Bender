...
namespace Tests.Legacy.Deserializer.Xml
{
    ...
    public class ListTests
    {
        ...
        public void should_deserialize_list_with_custom_name_format()
        {
            ...
            var result = Bender.Deserializer.Create(x => x.WithEnumerableTypeNameFormat("{0}s"))
        ...
        {
            const string xml = @"<ArrayOfAnyType><Int32>1</Int32></ArrayOfAnyType>";
            Assert.Throws<MappingException>(() => Bender.Deserializer
                .Create().DeserializeXml<IEnumerable>(xml))
        ...
        public void should_serialize_simple_type_array_list()
        {
            const string xml = @"<ArrayOfAnyType><Int32>1</Int32></ArrayOfAnyType>";
            Assert.Throws<MappingException>(() => Bender.Deserializer
                .Create().DeserializeXml<ArrayList>(xml))
                .InnerException.ShouldBeType<TypeNotSupportedException>();
        }
        ...
        public void should_not_deserialize_simple_type_enumerable_property()
        {
            ...
            Assert.Throws<MappingException>(() => Bender.Deserializer
                .Create().DeserializeXml<ListProperty>(xml))
                .InnerException.ShouldBeType<TypeNotSupportedException>();
        }
